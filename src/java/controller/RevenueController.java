...
public class RevenueController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        RevenueDao orderDAO = new RevenueDao();

        String filter = request.getParameter("filter");
        if (filter == null || filter.isEmpty()) {
            filter = "day";  // Mặc định là lọc theo ngày nếu không có tham số
        }
        
        Map<String, Integer> revenueData = orderDAO.getRevenueByDate(filter);
        int totalRevenue = revenueData.values().stream().mapToInt(Integer::intValue).sum();
