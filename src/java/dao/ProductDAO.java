...
public class ProductDAO extends DBContext {
    ...
    public ArrayList<Product> filterProductsByTitle(String searchTitle, int page, int pageSize, String sortBy, String sortOrder) {
        ArrayList<Product> productList = new ArrayList<>();

        // Xây dựng câu query cơ bản
        String sql = "SELECT p.pid, p.name, p.image, p.price, p.title, p.description, c.cname, c.cid, p.isDelete "
                + "FROM Product p JOIN Category c ON c.cid = p.cateID "
                + "WHERE p.isDelete = 0 AND p.title LIKE ? ";
        ...
        try (PreparedStatement stmt = connection.prepareStatement(sql)) {
            int paramIndex = 1;
            stmt.setString(paramIndex++, "%" + searchTitle + "%"); // Thêm wildcard % để tìm kiếm phần của title
...
// Thêm phương thức đếm tổng số sản phẩm phù hợp với title
    public int countProductsByTitle(String searchTitle) {
        String sql = "SELECT COUNT(*) FROM Product WHERE isDelete = 0 AND title LIKE ?";
        try (PreparedStatement stmt = connection.prepareStatement(sql)) {
